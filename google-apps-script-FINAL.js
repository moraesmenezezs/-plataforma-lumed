...
// VERSÃO FINAL - Trigger 1min + Sincronização manual
// ============================================
...
/**
 * WEB APP - Permite sincronização via URL
 * Chamado pelo botão "Sincronizar" no painel do professor
 */
function doGet(e) {
  // Executar sincronização
  const resultado = verificarNovasGravacoes();

  // Retornar resposta JSON com CORS
...
/**
 * Configura o trigger para executar a cada 1 MINUTO
 */
function configurarTrigger() {
  ...
  console.log('Trigger configurado para executar a cada 1 MINUTO');
}

/**
 * Limpa arquivo de vídeos processados (para testes)
 */
function limparProcessados() {
  try {
    ...
    if (arquivos.hasNext()) {
      const arquivo = arquivos.next();
      arquivo.setContent('{}');
      console.log('Arquivo de processados limpo');
    }
  } catch (e) {
...
/**
 * Função de teste - Lista todos os arquivos na pasta
 */
